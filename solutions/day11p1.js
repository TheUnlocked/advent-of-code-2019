...
const mapSize = 1000;
const map = new Array(mapSize).fill(0).map(x => new Array(mapSize).fill(paintTable.UNPAINTED));
const getMapPos = pos => map[pos[0] + (mapSize / 2)][pos[1] + (mapSize / 2)];
const setMapPos = (pos, v) => map[pos[0] + (mapSize / 2)][pos[1] + (mapSize / 2)] = v;
...
try {
    while (true) {
        cpu.inputs.push(getMapPos(botPos) == 1 ? 1 : 0);
        cpu.continue();
...
console.log(map.reduce((current, next, x) => current + next.filter((_, y) => map[x][y] != -1).length, 0));